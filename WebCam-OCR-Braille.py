...
class MIMOBraille(tk.Tk):
    ...
    def quit_button_pressed(channel,self):
        GPIO.cleanup()
...
#         camera.sharpness= int(Sharpness)  
        while True:
            ...
            cv2.imshow("Image to Braille", frame)
            cv2.moveWindow("Image to Braille",0,0)
            ...
            if key == 115:
                text = pytesseract.image_to_string(frame, config='-l eng --psm 1')
#                 print(text)
...
if __name__ == "__main__":
    app = MIMOBraille()
    app.title("Image Capture to Braille")
    setup()
    app.mainloop()
